...
def classify(name_pkl):
    ...
    way = Path(
        "/home/iury/HD/UFRPE/Mestrado_IA/VISAO/db_images/handwritten_digits/test"
    )
...
def fitness_cv(self):
    ...
    for chromosome in chromosomes:
        ...
        if not chromosome_resp:
            ...
            name_pkl = "/home/iury/HD/UFRPE/Mestrado_IA/VISAO/project/IA/digits_clf_inter_all.pkl"
            ...
            for path in paths:
                ...
                alelo_pipe_args = [
                    ...
                    (
                        cv2.threshold,
                        dict(
                            thresh=127, maxval=255, type=cv2.THRESH_BINARY_INV
                        ),
                    ),
                    (
                        cv2.threshold,
                        dict(thresh=127, maxval=255, type=cv2.THRESH_BINARY),
                    ),
                    (
                    ...
                    (cv2.erode, dict(kernel=np.ones((5, 5)), iterations=1)),
                    (cv2.erode, dict(kernel=np.ones((3, 3)), iterations=1)),
                    (cv2.dilate, dict(kernel=np.ones((5, 5)), iterations=1)),
                    (cv2.dilate, dict(kernel=np.ones((3, 3)), iterations=1)),
                    (
                        cv2.morphologyEx,
                        dict(op=cv2.MORPH_OPEN, kernel=np.ones((5, 5))),
                    ),
                    (
                        cv2.morphologyEx,
                        dict(op=cv2.MORPH_OPEN, kernel=np.ones((3, 3))),
                    ),
                    (
                        cv2.morphologyEx,
                        dict(op=cv2.MORPH_CLOSE, kernel=np.ones((5, 5))),
                    ),
                    (
                        cv2.morphologyEx,
                        dict(op=cv2.MORPH_CLOSE, kernel=np.ones((3, 3))),
                    ),
                ]

                pipe_args = [(cv2.cvtColor, dict(code=cv2.COLOR_BGR2GRAY))]

                list_genes = chromosome_str.split("-")
                for _gene in list_genes:
                    pipe_args.append(alelo_pipe_args[int(_gene)])
                ...
                list_count_error.append(
                    len(label_test)
                    - len(
                        [
                            True
                            for x, y in zip_longest(label_test, cls)
                            if x == y
                        ]
                    )
                )

                print(f"counter_error: {sum(list_count_error)}", end="\n\n")

            chromosome_resp = sum(list_count_error)
